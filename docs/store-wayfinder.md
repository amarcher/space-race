...
| 1 | Open accounts: a **new, dedicated** Stripe account for Space Race (Fable Designer LLC — do not touch `storybook-studio`'s existing account), Shippo, Resend domain for `spaceexplorer.tech` | ⬜ next — blocked on the Fable Designer EIN (fax not yet sent as of 2026-08-12); needs Andrew directly either way (KYC/business details, can't be done by an agent) |
...
- Stripe dashboard: _TBD_ (new, dedicated Fable Designer account — separate
  from `storybook-studio`'s live one)
