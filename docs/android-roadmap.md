...
## Phase 2 — Make it feel native (Android)

Most of "native feel" is already done cross-platform by the iOS work. The
Android-only gaps:

- [ ] **Hardware / gesture BACK button** — the single biggest Android-native
      gap. By default Capacitor lets Back exit the app instantly, which feels
      broken mid-game. Add `@capacitor/app` and a `backButton` listener:
      dismiss open overlays/menus first; on the root game view, either no-op or
      "press Back again to exit." Enable the Android 14+ **predictive back**
      gesture (`android:enableOnBackInvokedCallback="true"` in the manifest).
- [ ] **Adaptive launcher icon** — replace the Capacitor placeholder with the
      planet mark (same source as the iOS icon): foreground + `#07071a`
      background layers in `mipmap-anydpi-v26/`, a **monochrome** layer for
      Android 13+ themed icons, and legacy density fallbacks. (Android Studio's
      Image Asset Studio or `@capacitor/assets` can generate the full set.)
- [ ] **Splash screen** — Android 12+ SplashScreen API (the
      `core-splashscreen` dependency is already present). Set
      `windowSplashScreenBackground = #07071a` and the planet icon in
      `themes.xml` so the boot is continuous with the starfield, matching the
      iOS dark launch storyboard.
- [ ] **Status bar + edge-to-edge** — the existing `@capacitor/status-bar`
      boot call (`Style.Dark` = light icons) already applies. But **API 35
      (Android 15) enforces edge-to-edge** for `targetSdk 35+`: the WebView draws
      under the status/nav bars, so the app must consume window insets. The iOS
      Phase 2 already added `env(safe-area-inset-*)` CSS; plumb the Android insets
      into those same custom properties (Capacitor exposes them, or set them from
      the inset listener) so nothing hides behind the system bars.
- [ ] **Killed webview artifacts** — the iOS CSS (`overscroll-behavior: none`,
      `touch-action: manipulation`, no text selection/callout on the game
      surface) is platform-neutral and already applies; just verify no
      Android-only long-press/selection or pull-to-refresh leaks through.
- [ ] **Haptics** — `@capacitor/haptics` uses the Android `Vibrator` (adds the
      `VIBRATE` permission automatically). Verify the same seams that buzz on iOS
      buzz on a real Android device (Simulator/emulator can't vibrate).

**Exit criteria:** a stranger handling the phone can't tell it's a webview, and
Back behaves like a native app.
